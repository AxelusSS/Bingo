...
c = c.replace('pool.add(new PoolEntry(Material.SPEAR, Difficulty.HARD));', '''pool.add(new PoolEntry(Material.WOODEN_SPEAR, Difficulty.MEDIUM));
...
with open('pom.xml', 'r', encoding='utf-8') as f:
    pom = f.read()
pom = pom.replace('<version>1.5.0</version>', '<version>1.5.1</version>')
...
with open(doc_path, 'r', encoding='utf-8') as f:
    doc = f.read()
doc = doc.replace('V1.6.0/bingo-1.6.0.jar', 'V1.5.1/bingo-1.5.1.jar')
doc = doc.replace('V1.6.0/BingoPack.zip', 'V1.5.1/BingoPack.zip')
doc = doc.replace('V1.6.0/server-icon.png', 'V1.5.1/server-icon.png')
